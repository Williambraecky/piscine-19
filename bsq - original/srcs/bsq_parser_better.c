...
t_bsq	*ft_create_bsq_info(void)
{
	...
	if (bsq_info)
	{
		bsq_info->height = (int)NULL;
		bsq_info->width = (int)NULL;
		bsq_info->empty = (char)NULL;
		bsq_info->obstacle = (char)NULL;
		bsq_info->full = (char)NULL;
	}
	return (bsq_info);
}
...
int		ft_parse_line(int fd, int line)
{
	...
	while (j < g_bsq_info->width)
	{
		if (!ft_check_char_validity(g_board->data[line][j]))
			return (INVALID_MAP);
		o_count += g_board->data[line][j] == g_bsq_info->obstacle;
		g_board->solving[line][j] = o_count +
			(line == 0 ? 0 : g_board->solving[line - 1][j]);
		j++;
	}
